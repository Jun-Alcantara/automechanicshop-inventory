# AMSPOS-2: Setup WatermelonDB Schema and database.ts
...
Define the complete WatermelonDB schema for all 13 tables and create `src/services/database.ts` as the single WatermelonDB entry point. This file exports the `database` instance and is the only file that imports from `@nozbe/watermelondb` (except model class files).
...
### 2. Create `src/services/schema.ts`

Define all 13 table schemas exactly matching `documents/data_model.md`:
...
- `database.ts` is the **only** file that imports from `@nozbe/watermelondb` except for model class files
- No screen, store, or hook ever imports `database` directly — only services import from `database.ts`
- Schema version starts at `1`; migrations are added in future tasks as needed

---

## Acceptance Criteria

- [ ] `src/services/schema.ts` defines all 13 tables matching `documents/data_model.md` exactly
- [ ] All `isIndexed: true` columns from the data model are marked in the schema
- [ ] `src/services/database.ts` exports a single `database` instance
- [ ] The adapter uses JSI mode (`jsi: true`)
